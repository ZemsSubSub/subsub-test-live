...
## Reports — пагинация и имена (обновлено)

- Пагинация живёт внизу карты таблицы: слева `Page [1] of N` и `Rows per page [10]`, справа `«` `‹` номера страниц с `…` `›` `»`. Строки скроллятся выше, пагинация всегда видна; горизонтального скролла нет — колонки всегда влезают.
- Размер страницы: 10 по умолчанию, варианты 5 / 10 / 25 / 50.
- Имя отчёта — `{коллекция или канал} {тип} {период}`, например `Crypto Basic July 2026`, `News UA - Big Media Deep May – July 2026`. По этому же правилу названы строки-затравки.

## Reports — данные и пагинация (обновлено)

- В затравке 50 отчётов в `Market insights` (коллекции × типы × периоды, пара в статусе `In progress`) и 18 в `My performance` — пагинация работает по-настоящему: 5 страниц по 10.
- Горизонтальный скролл появляется только когда содержимому реально не хватает места.

## Create Collection — единая модалка (новое)

- Одна кнопка `Create Collection` на Basic data и My collections. Понятие «Add to base» из интерфейса убрано.
- Модалка: имя коллекции сверху вне табов, три входа сегментом — `Paste links` · `Find in base` · `Describe with AI`, общий футер `Cancel` / `Create collection`. Переключение таба ничего не сбрасывает: ссылки, отметки в базе и промпт уходят в одну коллекцию.
- Тарифы (`localStorage.subsub_plan`: explorer / pro / business / enterprise, дефолт pro): вместимость коллекции 10 / 25 / 100 / ∞, число коллекций 3 / 5 / 15 / ∞.
- Подсказка лимита с инлайн-апселлом под панелями: в `Describe with AI` — «подберём до N каналов», в остальных — сколько мест осталось; если каналов выбрано больше, чем влезает, честно пишем сколько добавится. Кнопка не блокируется.
- Если коллекций уже максимум по тарифу — поле имени скрыто, доступен только выбор существующей коллекции + апселл.
- После создания коллекция открывается в новой табе; если был промпт — параллельно стартует sourcing (пилюля в шапке).
