...
- В строке остаются иконки  и , остальное — под  (меню как у файла в Media Library):  ·  —  —  (красный). Пока отчёт , все три кнопки неактивны.
  -  уводит к файлу в Media Library и сразу открывает его шеринг () — у отчётов нет своего шеринга, файл живёт в библиотеке.
  -  открывает модалку с тем же типом и коллекцией/каналом, период выбирается заново.
- Строки и отступы как в таблице коллекций (56px, первая ячейка 16px), бейджи статусов того же размера.
- Сортировка по статусу — по смыслу, а не по алфавиту: сверху то, что в работе ( /  / ), затем , , .
